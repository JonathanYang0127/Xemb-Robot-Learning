...
# Tab 1: Run moonlight
open_tab_and_run "echo 'Starting moonlight...'; moonlight"

# Give a moment for the first tab to initialize
sleep 1

# Tab 2: Source setup.bash and run roslaunch
open_tab_and_run "echo 'Launching ROS nodes...'; source ~/interbotix_ws/devel/setup.bash && roslaunch aloha master_both.launch"

# Give a moment for the second tab to initialize
sleep 1

# Tab 3: Activate conda environment and run the relay script
open_tab_and_run "echo 'Activating conda environment and starting relay script...'; source ~/miniconda3/etc/profile.d/conda.sh && conda activate aloha && cd ~/interbotix_ws/src/xemb_aloha/xemb_scripts && python relay_master_joints_full.py"
