...
def scale_value(value, min_val=-3, max_val=3):
    ...
    # Allow values outside [0, 1] range - no hard limit enforcement
...
def pack_data_for_udp(joint_values_left, joint_values_right, gripper_value_left, gripper_value_right, heartbeat, key_states=None, no_heartbeat=False):
    """
    Pack robot and key states into a dictionary for UDP transmission.
    
    Args:
        joint_values_left: List of 6 joint values for left arm (or None)
        joint_values_right: List of 6 joint values for right arm (or None)
        gripper_value_left: Left gripper value (or None)
        gripper_value_right: Right gripper value (or None)
        heartbeat: Current heartbeat counter (0-3)
        key_states: Dictionary of key states
        no_heartbeat: If True, pack all data in every packet for higher frequency
    ...
    if no_heartbeat:
        # No heartbeat mode: pack all data in every packet
        data_packet['no_heartbeat'] = True
        
        # Pack all left arm joints
        if joint_values_left:
            data_packet.update({
        ...
        # Pack all right arm joints
        if joint_values_right:
            data_packet.update({
        ...
        # Pack grippers
        if gripper_value_left is not None:
            data_packet['gl'] = scale_value(gripper_value_left)
        if gripper_value_right is not None:
            data_packet['gr'] = scale_value(gripper_value_right)
    else:
        # Original heartbeat mode: pack data based on heartbeat counter
        if heartbeat == 0: # Left arm joints 1-3
            ...
            if joint_values_left and gripper_value_left is not None:
                data_packet.update({
                    'j4l': scale_value(joint_values_left[3]),
                    'j5l': scale_value(joint_values_left[4]),
                    'j6l': scale_value(joint_values_left[5]),
            ...
            if joint_values_right and gripper_value_right is not None:
                data_packet.update({
                    'j4r': scale_value(joint_values_right[3]),
                    'j5r': scale_value(joint_values_right[4]),
                    'j6r': scale_value(joint_values_right[5]),
...
def unpack_data_from_udp(data_packet):
    ...
    # Check if this is a no-heartbeat packet (contains all data)
    if data_packet.get('no_heartbeat', False):
        # No heartbeat mode: extract all available joint data
        ...
        # Original heartbeat mode: extract data based on heartbeat counter
        if heartbeat == 0:
            if 'j1l' in data_packet: joints_left[0] = unscale_value(data_packet['j1l'])
            if 'j2l' in data_packet: joints_left[1] = unscale_value(data_packet['j2l'])
            if 'j3l' in data_packet: joints_left[2] = unscale_value(data_packet['j3l'])
