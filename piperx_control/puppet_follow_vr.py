#!/usr/bin/env python3
"""
PiperX VR Puppet Robot Control Script
Receives VR controller data and performs Cartesian control using inverse kinematics
"""
...
from packing_utils_vr import unpack_vr_data_from_udp
...
LINEAR_VELOCITY_SCALE = 1   # Scale factor for linear movement
ANGULAR_VELOCITY_SCALE = 1.0  # Scale factor for angular movement
...
def process_auxiliary_controls(button_states, stepper, base, last_button_states=None):
    """Process auxiliary controls for stepper and base via button presses"""
    ...
    # Base control using thumbstick (could be extended for other buttons)
    if button_states.get('thumbstick', False) and not last_button_states.get('thumbstick', False):
        if base and hasattr(base, 'turn_left'):
            base.turn_left(0.2)
        elif base:
            print("Mock: Base turn left")
...
class PiperXVRController:
    ...
    def _handle_reset_button(self, button_states):
        """Handle reset button (X button) press with edge detection."""
        # Set a flag to track if a reset is in progress
        if not hasattr(self, 'reset_in_progress'):
            self.reset_in_progress = False

        # Check for reset button (X button) - PRIORITY COMMAND
        # Use edge detection: trigger only on the initial press
        is_reset_pressed = button_states.get('x_button', False)
        if is_reset_pressed and not self.last_reset_state:
            self.reset_in_progress = True
            print(f"[Robot {self.robot_id}] 🔄 RESET BUTTON (X) PRESSED - Moving to neutral position!")
...
def parse_arguments():
    """Parse command line arguments for VR puppet control."""
    parser = argparse.ArgumentParser(description='PiperX VR Puppet Robot Control')
    parser.add_argument('--single-arm', action='store_true', help='Use only one arm')
    parser.add_argument('--can-left', default='can0', help='CAN interface for left arm (default: can0)')
    parser.add_argument('--can-right', default='can1', help='CAN interface for right arm (default: can1)')
...
def configure_global_settings(args):
    ...
    print("🤖 Initializing PiperX VR Puppet Robot...")
...
def wait_for_vr_initialization(sock):
    """Wait for VR controller initialization handshake."""
    ...
    while True:
        try:
            ...
            if packet.get('mode') == 'vr_init':
                session_id = packet.get('init_session_id')
                robot_id = packet.get('robot_id', 0)
                
                print(f"✅ [INIT] VR controller connected! Session: {session_id[:8]}..., Robot: {robot_id}")
                print(f"🔧 DEBUG: Full session ID: {session_id}")
                print(f"🔧 DEBUG: Assigned robot ID: {robot_id}")
                
                # Send acknowledgment
                ack_packet = {'mode': 'vr_init_ack'}
                print(f"🔧 DEBUG: Sending acknowledgment: {ack_packet}")
    ...
    return session_id, robot_id

def print_control_instructions():
    """Print VR control instructions for the user."""
    print("🚀 Starting VR teleoperation...")
    print("🎯 Control Instructions:")
    print("   - Squeeze button: 🎯 MOVEMENT (hold to move robot)")
    print("   - Trigger: 🤏 GRIP (press=close, release=open)")
    print("   - X button: 🔄 RESET to neutral position") 
    print("   - Y button: (Available for future features)")
    print("   - A button: (Available for future features)")
    print("   - B button: (Available for future features)")
    print("   - Menu button: Stepper up")
    print("   - Thumbstick: Base turn")
    print("   - Ctrl+C: Exit")

def process_vr_packet(packet, session_id, vr_connected, puppet_left, puppet_right, 
                     stepper, base, last_button_states, args):
    """Process a single VR packet and update robots."""
    # Filter packets by session and mode
    if (packet.get('mode') != 'vr_teleop' or 
    ...
    if DEBUG_MODE and position is not None and orientation is not None:
        print(f"🎮 PUPPET RECEIVED: pos=[{position[0]:.3f},{position[1]:.3f},{position[2]:.3f}] "
    ...
    if vr_connected:
        ...
        # Process auxiliary controls
        if button_states != last_button_states:
            last_button_states = process_auxiliary_controls(
                button_states, stepper, base, last_button_states
            )
        
        # Debug output
        if args.debug:
            print(f"🐛 VR: Pos=[{position[0]:.3f},{position[1]:.3f},{position[2]:.3f}] "
                  f"Trigger={trigger_value:.2f} Active={trigger_active}")
    else:
        print("⚠️ VR DISCONNECTED - Skipping command processing")
    
    return True, last_button_states

def run_teleoperation_loop(sock, session_id, puppet_left, puppet_right, stepper, base, args):
    ...
    while True:
        ...
        try:
            ...
            if time.time() - last_vr_data_time > vr_connection_timeout:
                if vr_connected:
                    print("❌ VR CONNECTION LOST - Robot will stop responding to prevent erratic behavior")
                    vr_connected = False
        ...
        if time.time() - last_status_time > 5.0:
            ...
            print(f"📊 VR Puppet Status: {rate:.1f} Hz")
            packet_count = 0
            last_status_time = time.time()
...
def cleanup_resources(puppet_left, puppet_right, sock):
    ...
    sock.close()
    print("🔚 VR puppet control terminated")

def main():
    """Main function for PiperX VR puppet control."""
    ...
    stepper, base = initialize_auxiliary_controllers(args)
    sock = setup_vr_communication()
    
    try:
        # VR initialization handshake
        session_id, robot_id = wait_for_vr_initialization(sock)
        
        # Run main teleoperation loop
        run_teleoperation_loop(sock, session_id, puppet_left, puppet_right, stepper, base, args)
        
    except KeyboardInterrupt:
        print("\n🛑 VR teleoperation stopped by user")
    except Exception as e:
        print(f"❌ VR teleoperation error: {e}")
        import traceback
        traceback.print_exc()
    finally:
        cleanup_resources(puppet_left, puppet_right, sock)
